    ...
    pub fn initialize(
        ...
        if let Some(alloc_limits) = alloc_limits {
            ...
            std::thread::spawn(move || {
                // Initialization races are possible here: allocator thread can start
                // before chunk sizes are configured. Wait until the divisor is valid.
                let min_chunks_count = loop {
                    let chunk_usable_size = self.chunk_usable_size.load(Ordering::Relaxed);
                    if chunk_usable_size > 0 {
                        break alloc_limits.min_fs_usage.as_bytes() / chunk_usable_size;
                    }
                ...
                while self.is_active.load(Ordering::Relaxed) {
                    ...
                    if let Ok(stats) = ProcessStats::get() {
                        if stats.memory_usage_bytes as usize > alloc_limits.max_usage.as_bytes() {
                            ...
                            let mut chunks_to_free = vec![];
                            {
                                ...
                                while chunks.len() > MIN_FREE_CHUNKS_COUNT
                                    && ((stats.memory_usage_bytes as usize).saturating_sub(
                                        chunks_to_free.len() * chunk_padded_size,
                            ...
                            let allowed_chunks = allowed_space / chunk_padded_size;
                            if allowed_chunks > 0 {
                                let mut cleared_chunks = self.cleared_chunks.lock();
                                let mut chunks = self.chunks.lock();
